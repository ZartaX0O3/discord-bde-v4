...
module.exports = {
    ...
    run: async (client, message, args) => {
        try {
            ...
            if(!user || user.id == null || !user.id) message.reply("❌ Impossible de trouver l'utilisateur")
            try{
                const member = message.guild.members.cache.get(user.id);
                //create the EMBED
                const embeduserinfo = new MessageEmbed()
                embeduserinfo.setThumbnail(member.user.displayAvatarURL({ dynamic: true, size: 512 }))
                embeduserinfo.setAuthor("Permissions de :   " + member.user.username + "#" + member.user.discriminator, member.user.displayAvatarURL({ dynamic: true }), "https://clan.milrato.eu")
